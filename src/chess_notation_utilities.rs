...
pub fn get_bounds (spot: &str) -> bounds {
  let top = {
        match &spot.chars().nth(1).unwrap(){
            '8' => None,
            row_char @ '1' | row_char @ '2' | row_char @ '3' 
          | row_char @ '4' | row_char @ '5' | row_char @ '6'| row_char @ '7' => {
  ...
  let bottom = {
    match &spot.chars().nth(1).unwrap(){
        '1' => None,
        row_char @ '8' | row_char @ '7' | row_char @ '6' 
      | row_char @ '5' | row_char @ '4' | row_char @ '3'| row_char @ '2' => {
            if let Some(first_char)  =  spot.chars().nth(0) {
                let mut row = row_char.to_digit(10).unwrap();
                row -= 1;
                let y= format!("{}{}",first_char,row);
...
let left = {
    match &spot.chars().nth(0).unwrap(){
        col_char @'b' | col_char @'c'| col_char @'d'| col_char @'e' 
        | col_char @'f' | col_char @'g' | col_char @'h' =>  {
...
let right = {
    match &spot.chars().nth(0).unwrap(){
        col_char @'a' | col_char @'b'| col_char @'c'| col_char @'d' 
        | col_char @'e' | col_char @'f' | col_char @'g' =>  {
            if let Some(second_char)  =  spot.chars().nth(1) {
  ...
  let (top_left_diag,top_right_diag, bottom_left_diag, bottom_right_diag)=(None,None,None,None);
...
pub fn convert_move_notation_to_indexes( from_spot: &str, to_spot: &str) ->Result<(usize,usize),&'static str>{
