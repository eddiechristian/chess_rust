...
pub trait GamePiece{
    ...
    fn move_forward_one(&self, pos: &str, state: &GameState )->Result<(),&'static str>;
...
impl GamePiece for Pawn{
    ...
    fn move_forward_one(&self, from_pos: &str, state: &GameState )->Result<(),&'static str>
    {
        match self.player {
            PLAYER::WHITE => {
                if let Some(to_pos_array) = chess_notation_utilities::get_bounds(from_pos).top{
                    let to_pos = std::str::from_utf8(&to_pos_array).unwrap();
                    println!("{:?}",to_pos);
